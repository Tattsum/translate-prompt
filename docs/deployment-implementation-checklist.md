...
- [ ] Access: SPA URL アプリ + ポリシー（[手順](./deployment-access-setup.md)）← **次の手動作業**
...
- [x] SPA Worker 経由 `translate.tattsum.com` 200
- [ ] Access ログイン → SPA 表示（Access 設定後）
- [x] Estimate / Analyze / Optimize 成功（API 直接確認済み）
...
| WP-4 | 完了 | |
| WP-5 | **進行中** | Access 手動設定のみ残 |
| WP-6 | ほぼ完了 | Access 設定後のブラウザ確認のみ残 |
| WP-7 | 完了 | |
