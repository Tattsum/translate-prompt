...
| **SPA Worker** | `https://translate.tattsum.com` | **200** | HTTP GET（DNS A + Worker Route 反映済み） |
...
| コンポーネント | URL | 状態 | 原因 |
|----------------|-----|------|------|
| Cloudflare Access | SPA + API | **未設定** | Zero Trust ダッシュボードで手動設定が必要 |
...
| 4 | **Cloudflare Access 設定** | [deployment-access-setup.md](./deployment-access-setup.md) | **手動待ち** ← 次 |
| 5 | **E2E スモークテスト** | `./scripts/deployment-smoke-test.sh` + ブラウザ確認 | Access 後 |
...
1. docs/deployment-session-handoff.md
2. docs/deployment-access-setup.md  ← 次の手動作業
3. docs/deployment-implementation-checklist.md

## 現状サマリ（2026-07-03）

- WP-1〜4 + DNS + SPA Worker: 完了
- translate.tattsum.com → 200（Workers プロキシ経由）
- prompt-api.tattsum.com → health / Estimate / Analyze OK
- Cloudflare Access: **未設定**（現状は認証なしでアクセス可能）

## 今回のゴール

Cloudflare Access を設定し、招待制 β として公開を完了する。

### 具体的タスク

1. Zero Trust で IdP 追加（Google / GitHub）
2. Application `translate-prompt-spa`（translate.tattsum.com）
3. Application `translate-prompt-api`（prompt-api.tattsum.com）
4. ブラウザで Analyze → Optimize 動作確認
5. ./scripts/deployment-smoke-test.sh 実行
```
