# Web 公開デプロイ — セッション引き継ぎ（2026-07-02）
...
| SPA 公開 URL | `https://translate.tattsum.com` | **コード実装済み**（Workers プロキシ）。DNS 手動設定待ち |
...
| Pages（デフォルト） | `https://translate-prompt.pages.dev` | 200 | HTTP GET |
| Pages（別名） | `https://translate-prompt-2el.pages.dev` | 200 | HTTP GET |

### 3.2 未達・問題あり

| コンポーネント | URL | 状態 | 原因 |
|----------------|-----|------|------|
| SPA カスタムドメイン | `https://translate.tattsum.com` | **522** | Pages カスタムドメイン banned + DNS のみ設定（Pages 側未紐付け） |
| SPA カスタムドメイン（旧案） | `https://prompt.tattsum.com` | **banned** | Cloudflare Pages が `tattsum.com` サブドメインを拒否 |
| API カスタムドメイン（旧案） | `https://api.prompt.tattsum.com` | **TLS handshake failure** | 入れ子サブドメインは Universal SSL 非対象 |
...
| 9 | `translate.tattsum.com` 522 | DNS CNAME のみ。Pages カスタムドメイン UI で登録できない | Workers プロキシ Worker 実装済み。DNS 手動切替待ち | **進行中** |
| 10 | Cloudflare Access | 未設定 | [deployment-access-setup.md](./deployment-access-setup.md) | **次の手動作業** |
...
| 3 | **Cloudflare Access 設定** | [deployment-access-setup.md](./deployment-access-setup.md) | 手動待ち |
| 4 | **DNS 整理** | [deployment-dns-setup.md](./deployment-dns-setup.md) | 手動待ち |
| 5 | **E2E スモークテスト** | `./scripts/deployment-smoke-test.sh` | DNS/Access 後 |
...
| # | 確認項目 | 期待結果 | 状態 |
|---|---------|---------|------|
| 1 | 未認証で SPA URL にアクセス | Access ログイン画面 | 未実施（Access 未設定） |
| 2 | 認証後 SPA 表示 | Input ページ | 未実施 |
| 3 | Health | `prompt-api.tattsum.com` 経由で `ok` | **OK** |
| 4 | Estimate | トークン数が返る | 未実施 |
| 5 | Analyze | 質問または ready | 未実施 |
| 6 | Optimize | 最適化結果 | 未実施 |
| 7 | Investigate（Web） | 403 / エラー | 未実施 |
| 8 | CLI Investigate | ローカルで動作 | 未確認 |
| 9 | CORS | 許可オリジン以外拒否 | 未実施 |
| 10 | Playground | 本番 404 | 未実施 |
...
| CORS 制限 | 設定済み（ただし SPA URL と不一致の可能性） |
| Access 招待制 | **未設定** |
| Fly オリジン非公開（Tunnel） | **未導入**（パブリック URL 直結） |
| GraphQL Playground 本番無効 | `ENV=production` で想定 |
...
```
translate-prompt の Web 公開（招待制 β）の続きを実装してください。

## 必読ドキュメント（この順で読むこと）

1. docs/deployment-session-handoff.md  ← 現状スナップショット（最優先）
2. docs/deployment.md                  ← 合意サマリが設計の正
3. docs/deployment-implementation-checklist.md

## 現状サマリ

- WP-1〜4（コード + CI）は MERGED 済み（PR #1）
- API は動作: https://prompt-api.tattsum.com/query （health ok）
- Fly 直: https://translate-prompt-api.fly.dev
- Pages SPA: https://translate-prompt.pages.dev （200）
- translate.tattsum.com は 522（Pages カスタムドメイン banned）
- Cloudflare アカウント: Kurohari35@gmail.com's Account（CI の Secrets はこちら）
- ローカル wrangler は別アカウントのためローカル deploy はゾーンエラー。CI 経由でデプロイすること

## 今回のゴール

SPA を tattsum.com 配下で公開できるようにする。blog（tattsum.com → Worker "blog"、OpenNext）と同様に、
**Pages カスタムドメインは使わず Workers 経由**で公開する方式 A（Pages プロキシ Worker）を実装する。

### 具体的タスク

1. 新 Worker `translate-prompt-web`（または同等）を追加
   - translate-prompt.pages.dev へのリバースプロキシ
   - wrangler 設定 + deploy.yml にデプロイジョブ追加
2. DNS / Workers Route で translate.tattsum.com を新 Worker に紐付け
   - Pages カスタムドメイン UI は使わない
   - 既存の translate CNAME（Pages 向け）は整理
3. fly.toml の ALLOWED_ORIGINS を https://translate.tattsum.com に合わせる（Fly secrets も）
4. Cloudflare Access の手順をドキュメント化（手動設定。アプリ 2 つ: SPA + API）
5. スモークテスト手順を実行可能な形で残す
6. docs/deployment.md と checklist を実態に合わせて更新

## 制約

- ローカル開発（make serve / make dev）は壊さないこと
- 合意サマリ（招待制 β、Investigate Web 無効、Fly + Pages + Workers）は維持
- Tunnel は今回は導入しない（Fly パブリック URL 直結のまま）
- コミットは明示的に依頼されるまで作らない

## 参考: blog パターン

tattsum.com は DNS タイプ Worker → "blog"（OpenNext + Next.js on Workers）。
Pages カスタムドメインではなく Workers で apex を配信している。
prompt-api.tattsum.com も Workers ルートで動作実績あり。
```
